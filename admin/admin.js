...
window.onSb(async (sb)=>{
  ...
  if(session) return enterDashboard(session.user);
...
async function enterDashboard(user){
  ...
  $('logout').onclick = async ()=>{ await window.sb.auth.signOut(); location.reload(); };
  ...
  $('modal-close').onclick = $('modal-cancel').onclick = ()=>{ $('prod-modal').style.display='none'; };
  $('p-file').addEventListener('change', async e=>{
    const f = e.target.files?.[0]; if(!f) return;
    showError('prod-err','Uploading…');
    try{ const url = await window.api.uploadImage(f); $('p-image').value = url; $('p-preview').style.backgroundImage=`url(${url})`; showError('prod-err',''); }
    catch(err){ showError('prod-err', err.message); }
  });
  $('p-image').addEventListener('input', e=>{ $('p-preview').style.backgroundImage = e.target.value?`url(${e.target.value})`:''; });
  $('prod-form').addEventListener('submit', saveProduct);

  $('cat-form').addEventListener('submit', async e=>{
    e.preventDefault(); showError('cat-err','');
    try{
      ...
      $('cat-form').reset();
      await refreshCats();
    }catch(err){ showError('cat-err', err.message); }
  });
...
async function refreshProducts(){
  ...
  $('prod-count').textContent = `${cachedProds.length} product${cachedProds.length===1?'':'s'}`;
